...
import shap
OUT="results/shap_seed_runs.csv"
...
for era in eras:
    ...
    for s in SEEDS:
        ...
        rf=RandomForestClassifier(n_estimators=150,max_depth=12,random_state=s,n_jobs=-1).fit(X,y)
        rng=np.random.RandomState(100+s); Xs=X[rng.choice(len(sub),min(200,len(sub)),replace=False)]
